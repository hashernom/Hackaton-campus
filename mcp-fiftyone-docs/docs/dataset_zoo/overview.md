...
Use [`load_zoo_dataset()`](../api/fiftyone.zoo.datasets.html#fiftyone.zoo.datasets.load_zoo_dataset "fiftyone.zoo.datasets.load_zoo_dataset") to load a zoo dataset into a FiftyOne dataset.

For example, the code sample below loads the validation split of [COCO-2017](datasets/coco_2017.html#dataset-zoo-coco-2017) from the zoo and visualizes it in the FiftyOne App:
...
Use [fiftyone zoo datasets load](../cli/index.html#cli-fiftyone-zoo-datasets-load) to load a zoo dataset into a FiftyOne dataset.
...
If you do not have a [suitable package](api.html#dataset-zoo-ml-backend) installed when attempting to download a zoo dataset, youâll see an error message that will help you install one.

## Built-in datasets#

The Dataset Zoo provides built-in access to dozens of datasets that you can load into FiftyOne with a single command.

[ Explore the datasets in the zoo ](index.html)

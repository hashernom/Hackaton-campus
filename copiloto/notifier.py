...
def notify(caption, photo_path=None, chat_id=None, token=None, respect_cooldown=True):
    ...
    if not token or not chat_id:
        print(f"[notifier][FALLBACK] (sin token/chat_id) {caption} :: {photo_path}")
    ...
    try:
        if photo_path:
            ...
            ok, info = r.ok, r.text
        if ok:
            print(f"[notifier] enviado a {chat_id}")
        else:
            print(f"[notifier][ERROR] Telegram respondió: {info}")
        _last_sent = time.time()
        return ok
    except Exception as e:
        print(f"[notifier][ERROR] no se pudo enviar: {e}")
        _last_sent = time.time()
        return False
